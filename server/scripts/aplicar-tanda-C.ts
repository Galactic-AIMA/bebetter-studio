/**
 * Tanda C — 11 frases CON HISTÓRICO: se archiva la original y entra una nueva.
 *
 ...
 *
 * Deshacer: `_tandaC-aplicada.json` guarda el id nuevo y el viejo de cada par.
 */
import fs from 'fs'
...
const MAPA = JSON.parse(fs.readFileSync(path.join(__dirname, '_tandaC.json'), 'utf-8')) as
...
console.log(`\n${APLICAR ? 'APLICANDO' : 'DRY-RUN'} — tanda C (${MAPA.length} frases, archivar + crear)\n`)
...
fs.writeFileSync(path.join(__dirname, '_tandaC-aplicada.json'), JSON.stringify(pares, null, 2), 'utf-8')
